...
bool chunk_visible(Planes &planes, const ChunkPos &pos, int miny, int maxy, bool ortho) {
    ...
    int d = CHUNK_SIZE + 1;
    int points[8][3] = {{x + 0, miny, z + 0}, {x + d, miny, z + 0}, {x + 0, miny, z + d}, {x + d, miny, z + d},
                        {x + 0, maxy, z + 0}, {x + d, maxy, z + 0}, {x + 0, maxy, z + d}, {x + d, maxy, z + d}};
    int n = ortho ? 4 : 6;
    for (int i = 0; i < n; i++) {
        int in = 0;
        int out = 0;
        for (int j = 0; j < 8; j++) {
            float d =
                planes(i, 0) * points[j][0] + planes(i, 1) * points[j][1] + planes(i, 2) * points[j][2] + planes(i, 3);
            if (d < 0) {
                out++;
            } else {
                in++;
...
void draw_chunk(Shader *attrib, Chunk *chunk) { draw_triangles_3d_ao(attrib, chunk->buffer, chunk->faces * 6); }
