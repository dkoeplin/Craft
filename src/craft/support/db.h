...
struct Database {
  public:
    ...
    int auth_get_selected(char *username, int username_length, char *identity_token, int identity_token_length);
    void save_state(float x, float y, float z, float rx, float ry);
    void save_state(const State &state) { save_state(state.x, state.y, state.z, state.rx, state.ry); }

    int load_state(float *x, float *y, float *z, float *rx, float *ry);
    int load_state(State &state) { return load_state(&state.x, &state.y, &state.z, &state.rx, &state.ry); }
    ...
    void delete_signs(const ILoc3 &loc) { delete_signs(loc.x, loc.y, loc.z); }

    void delete_all_signs();
    void load_blocks(Map *map, int p, int q);
    void load_blocks(Map *map, const ChunkPos &pos) { load_blocks(map, pos.x, pos.z); }

    void load_lights(Map *map, int p, int q);
    void load_lights(Map *map, const ChunkPos &pos) { load_lights(map, pos.x, pos.z); }

    void load_signs(std::vector<Sign> &list, int p, int q);
    void load_signs(std::vector<Sign> &list, const ChunkPos &pos) { load_signs(list, pos.x, pos.z); }

    int get_key(int p, int q);
    int get_key(const ChunkPos &pos) { return get_key(pos.x, pos.z); }

    static void set_key(int p, int q, int key);
    static void set_key(const ChunkPos &pos, int key) { set_key(pos.x, pos.z, key); }
