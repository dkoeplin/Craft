...
struct ChunkPos {
  ...
  template<int radius>
  void surrounding(const std::function<void(ChunkPos)> &func) const {
  ...
  template<int radius>
  void surrounding(const std::function<void(ChunkPos, int, int)> &func) const {
      for (int dx = -radius; dx <= radius; ++dx) {
          for (int dz = -radius; dz <= radius; ++dz) {
...
struct Vec3 {
  ...
  void set_pos(const Vec3<Index> &rhs) { x = rhs.x; y = rhs.y; z = rhs.z; }

  template <int radius>
  void horizontal(std::function<void(Vec3<Index>)> func) const {
      for (int dx = -radius; dx <= radius; ++dx) {
          for (int dz = -radius; dz <= radius; ++dz) {
              func({x + dx, y, z + dz});
          }
...
float distance(const Vec3<A> &a, const Vec3<B> &b) {
